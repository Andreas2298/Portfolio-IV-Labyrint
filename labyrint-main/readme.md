...
## Features (Tasks)
** Gjort. The starting level has an empty slot in the surrounding wall. This slot should function as a door into the level called "aSharpPlace." Implement the door functionality so that the player can proceed to the next level.  
** Gjort. Create a new level (a third level) and link the unused door in "aSharpPlace" to exit into the new room.  
** Gjort In "aSharpPlace," implement teleport functionality for the "♨︎" symbols. Entering one should move the player to the other.  
** Gjort. Ensure that when going back through a door, the player returns to the correct room.  
** Ikke gjort Make the X NPC characters perform a simple patrol (+/-2 from their starting locations).  
** Gjort. Create an animated splash screen (this was a group assignment from a previous week) using `splashScreen.mjs`.  

* Give the NPCs stats, such as strength and hitpoints.  
* Gjort. Implement a simple battle system where collisions deal damage, using player and NPC stats to calculate damage dealt.  
* Gjort. Output battle events to the event messages displayed beneath the map.  
* Have event messages remain on screen longer (currently, they only survive one update cycle).  
* Gjort. Create two new pickups: a health potion and poison, and include them in your level.  
